...
/**
 * SearchCodeTool：提供受工作区约束的具体文件操作工具。 所属层：基础设施层。
 *
 * @author yy
 */
public class SearchCodeTool extends BaseTool {
  ...
  public void validate(Object input) {
    ...
    if (toolInput.getKeyword() == null || toolInput.getKeyword().trim().isEmpty()) {

      throw new IllegalArgumentException("search_code keyword is empty");
    }
  }
  ...
  public Object execute(Object input, AgentContext context) throws Exception {
    ...
    String keyword = toolInput.getKeyword();
    ...
    File root = workspaceService.resolve(path);

    if (!root.exists()) {

      throw new RuntimeException("path not exists:" + root.getAbsolutePath());
    }

    List<SearchMatch> result = new ArrayList<>();

    scan(root, keyword, result);

    return result;
  }

  private void scan(File file, String keyword, List<SearchMatch> result) {
    ...
    searchFile(file, keyword, result);
  }

  private void searchFile(File file, String keyword, List<SearchMatch> result) {
    ...
    try {
      ...
      for (int i = 0; i < lines.size(); i++) {
        ...
        if (pattern.matcher(current).find() && !isImportLine(current) && !isPackageLine(current)) {
  ...
  private List<String> buildContext(List<String> lines, int index) {
