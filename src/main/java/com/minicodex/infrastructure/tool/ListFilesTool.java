...
/**
 * ListFilesTool：提供受工作区约束的具体文件操作工具。 所属层：基础设施层。
 *
 * @author yy
 */
public class ListFilesTool extends BaseTool {
  ...
  public Object execute(Object input, AgentContext context) throws Exception {
    ...
    if (!root.exists()) {
      ...
      throw new RuntimeException("directory not found:" + root.getAbsolutePath());
    }
  ...
  private void scan(File file, List<String> result) {
    ...
    if (file.isFile()) {
      ...
      result.add(workspaceService.relativePath(file));
