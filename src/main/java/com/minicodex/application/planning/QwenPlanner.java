...
import com.minicodex.application.project.ProjectContextService;
import com.minicodex.application.skill.SkillManager;
...
/**
 * QwenPlanner：负责生成和校验代码改造计划。 所属层：应用层。
 *
 * @author yy
 */
public class QwenPlanner implements Planner {
  ...
  private final PromptRepository promptLoader;
  ...
  public CodePlan createPlan(AgentContext context) {
    ...
    log.info(
        "planner start phase={} task={}", context.getPhase(), summarize(context.getTask(), 120));
    ...
    log.info(
        "========== PROMPT TAIL ==========\n{}",
        prompt.substring(Math.max(0, prompt.length() - 2000)));
    ...
    try {
      ...
      throw new RuntimeException("Planner generated invalid plan:" + e.getMessage());
    }

    log.info(
        "planner finish cost={}ms plan={}",
  ...
  private CodePlan repairExistingFileWritePlan(AgentContext context, CodePlan plan) {
    ...
    for (PlanStep step : plan.getSteps()) {
      ...
      if (oldText != null && newText != null) {
        log.warn("write existing file detected, convert to patch_file path={}", input.getPath());
      ...
      log.warn("write existing file detected, fallback to read_file path={}", input.getPath());
  ...
  private CodePlan repairInvalidPatchPlan(AgentContext context, CodePlan plan) {
    ...
    for (PlanStep step : plan.getSteps()) {
      ...
      if (!"patch_file".equals(step.getTool()) || !(step.getInput() instanceof ToolInput)) {
      ...
      if (shouldRepairPatchOldText(input)) {
        if (shrinkPatchText(input)) {
          log.warn("invalid patch oldText detected, shrink patch text path={}", input.getPath());
          continue;
        }

        log.warn("invalid patch oldText detected, fallback to read_file path={}", input.getPath());
  ...
  private boolean shrinkPatchText(ToolInput input) {
    ...
    try {
      ...
      PatchText patchText = buildMinimalPatchText(input.getOldText(), input.getNewText());
      if (patchText == null
      ...
      if (!content.contains(patchText.getOldText())
          && !normalizeLineSeparator(content)
              .contains(normalizeLineSeparator(patchText.getOldText()))) {
        return false;
      }
  ...
  private PatchText buildMinimalPatchText(String oldText, String newText) {
    ...
    return PatchText.builder().oldText(oldPatch).newText(newPatch).build();
  }
  ...
  private List<PlanStep> singleReadStep(String path) {
    ...
    steps.add(
        PlanStep.builder()
            .order(1)
            .tool("read_file")
            .input(ToolInput.builder().path(path).startLine(1).endLine(1000).build())
  ...
  private boolean isInvalidPatchOldText(String oldText) {
    ...
    String normalized = oldText.trim();
  ...
  private CodePlan parse(String task, String json) {
    try {
      if (json == null || json.trim().isEmpty()) {
        ...
        throw new RuntimeException("model response is empty");
      }
      ...
      return CodePlan.builder().task(task).steps(steps).build();
  ...
  private void validateModelResponse(JsonNode root) {
    ...
    throw new RuntimeException("model call failed:" + message);
  }
  ...
  private String buildObservations(AgentContext context) {
    ...
    for (int i = start; i < list.size(); i++) {
      ...
      if (o.getResult() != null) {
        ...
        int maxResultLength = "read_file".equals(o.getTool()) ? 8000 : 500;
