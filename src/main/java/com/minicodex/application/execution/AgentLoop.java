...
import com.minicodex.application.phase.PhaseManager;
import com.minicodex.application.planning.Planner;
import com.minicodex.application.project.ProjectIndexer;
import com.minicodex.application.verification.VerifyEngine;
...
import com.minicodex.domain.agent.AgentStatus;
...
/**
 * AgentLoop：负责 Agent 执行流程中的协调与结果汇总。 所属层：应用层。
 *
 * @author yy
 */
public class AgentLoop {
  ...
  private boolean shouldStopRepeatedFailure(
      List<ToolCallResult> results, Map<String, Integer> failureCounts) {
  ...
  private String failureSignature(List<ToolCallResult> results) {
    ...
    for (ToolCallResult result : results) {
      ...
      sb.append(result.getTool()).append(":").append(result.getError());
    }
