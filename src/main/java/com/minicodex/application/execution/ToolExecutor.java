...
import com.minicodex.application.phase.AgentPolicy;
...
/**
 * ToolExecutor：负责 Agent 执行流程中的协调与结果汇总。 所属层：应用层。
 *
 * @author yy
 */
public class ToolExecutor {
  ...
  public List<ToolCallResult> execute(CodePlan plan, AgentContext context) {
    ...
    List<Observation> currentObservations =
        context.getObservations().subList(beforeSize, context.getObservations().size());
    context.setLastObservations(new ArrayList<>(currentObservations));
    return results;
  }

  private void addFailure(
      List<ToolCallResult> results, AgentContext context, PlanStep step, String error) {
  ...
  private void executeTool(
      PlanStep step, AgentContext context, List<ToolCallResult> results, AgentTool tool) {
    try {
